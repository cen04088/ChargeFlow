...
"""
환경부 전기차 충전소 수집 스크립트 (수정판)
======================================================
문제 원인:
  환경부 API는 lat/lng/radius 파라미터를 지원하지 않음.
  → 시도별 전체 수집 후 Python에서 거리 계산으로 필터링

올바른 전략:
  1. 시도코드별로 전체 충전소 수집
  2. 각 IC 좌표와 거리 계산 (haversine)
  3. 3km 이내 + 15분 이내 조건으로 필터
  4. ChargingStation + NodeStationMapping 저장

사용법:
  cd chargeflow

  # 미리보기 (DB 저장 없음) — 경기도만 빠르게 테스트
  python scripts/collect_stations.py --api-key REDACTED_PUBLIC_DATA_API_KEY --zcode 41 --dry-run

  # 전체 수집
  python scripts/collect_stations.py --api-key REDACTED_PUBLIC_DATA_API_KEY
"""
...
def collect(api_key, zcode_filter, dry_run):
...
if __name__ == '__main__':
    ...
    collect(args.api_key, args.zcode, args.dry_run)
