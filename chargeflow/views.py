...
class BypassStationView(APIView):
    ...
    def get(self, request, pk):
        ...
        # 이전 IC: sequence < ra.sequence 중 가장 큰 것
        prev_ic = (
            ic_qs.filter(sequence__lt=ra_node.sequence)
                 .order_by('-sequence')
                 .first()
        )

        # 다음 IC: sequence > ra.sequence 중 가장 작은 것
        next_ic = (
            ic_qs.filter(sequence__gt=ra_node.sequence)
                 .order_by('sequence')
                 .first()
        )
